...
if (!isset($page) && !isset($pages)) {
    ...
    $count = count($items);
   	$pages = intval(ceil($count / $posts_per_page));
    echo $pages;
	} 
...
if ($pages > 1) {	
	...
	if ($page > 1) {
        ...
        $link     = ($type === 'sukien') ? get_pagenum_link(1) : get_comments_pagenum_link(1);
		$output[] = '<a class="first" href="'.$link.'">'.__('First', 'warp').'</a>';
        ...
        $link     = ($type === 'sukien') ? get_pagenum_link($page-1) : get_comments_pagenum_link($page-1);
		$output[] = '<a class="previous" href="'.$link.'">«</a>';
	}
