...
<div id="system" class="box2">
	...
	<?php if (have_posts()) : ?>
		...
		$post_id = get_the_ID();
          $taxonomy = wp_get_post_terms($post_id,'chude');
          $cat_name = $taxonomy[0]->name;
      ...
      <p><h3 style="padding-left:40px;">Bạn đã có thể bắt đầu đăng ký các hội thảo/khóa học tổ chức trong tháng 8/2013</h3></p>
		...
		<?php
          ...
          if(isset($_GET['act']) && $_GET['act']  ==  'su-kien-da-dien-ra'):
          ?>
            <article class="item" style="position:relative" data-permalink="<?php the_permalink(); ?>">
            <header>
              <h1 class="title" style="color:#fff;background:#f78000;text-align:center;-webkit-border-top-left-radius: 5px;
-webkit-border-top-right-radius: 5px;
-moz-border-radius-topleft: 5px;
-moz-border-radius-topright: 5px;
border-top-left-radius: 5px;
border-top-right-radius: 5px;width:858px"><?php echo $cat_name; ?></h1>
            </header>
...
<?php echo do_shortcode("[FAQ_LIST pageid='".$post_id."']");?>
        ...
        $cat_name = get_term_by('slug',get_query_var('chude'),'chude')->name;
        ?>
                    <article class="item" style="position:relative;min-height:250px" data-permalink="<?php the_permalink(); ?>">
            <header>
              <h1 class="title" style="color:#fff;background:#da7623;text-align:center;-webkit-border-top-left-radius: 5px;
-webkit-border-top-right-radius: 5px;
-moz-border-radius-topleft: 5px;
-moz-border-radius-topright: 5px;
border-top-left-radius: 5px;
